...
in the hardware addesses used by AX.25, which are based on the expectation of
...
This provides valuable information to the rest of the APRS network since APRS
...
while APRS imposes a looser standard that the SSID is only limited to 
...
This is because AX.25 doesn't guarantee delivery from a digipeater to another station,
so packets passing through a digipeater that are lost need to be resent from their origin,
where a higher layer protocol could recover from a lost packet locally without twice consuming
...
excesive burden on the APRS network. Digipeaters rarely have information of value
...
\begin{itemize}
	...
	\item There is no standard secondary place for a station's ``real" callsign 
		...
		Operators often program digipeaters with unclear comments that fail to make it clear
